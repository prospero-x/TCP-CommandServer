...
static void ParameterSweepLong(uint8_t Address, uint16_t* param_nums, uint32_t* param_vals);
static void ParameterSweepFloat(uint8_t Address, uint16_t* param_nums, float* param_vals);
static void ParameterSetLong(uint8_t Address, uint16_t* param_nums, uint32_t* param_vals);
static void ParameterSetFloat(uint8_t Address, uint16_t* param_nums, float* param_vals);
...
uint32_t* ParamValLongInit(){
    uint32_t* rv = (uint32_t*)malloc(sizeof(uint32_t)*41);
    return rv;

}

float* ParamValFloatInit(){
    float* rv = (float*)malloc(sizeof(float)*90);
    return rv;
}
...
int main()
{
    ...
    */
    ...
    int32_t Address = 0;
    uint16_t* ParamNuml = ParamNumLongInit();
    uint16_t* ParamNumf = ParamNumFloatInit();
    uint32_t* ParamVall = ParamValLongInit();
    float* ParamValf = ParamValFloatInit(); 
    ParameterSweepLong(Address, ParamNuml, ParamVall);
    ParameterSweepFloat(Address, ParamNumf, ParamValf);
    ...
    while(1)
      {
        switch(MenuSelection())
        {
            ...
            case 54:
            {
                ParameterSetLong(Address, ParamNuml, ParamVall);
                ParameterSetFloat(Address, ParamNumf, ParamValf);
            }
            default:
                ConsoleIO_SetColor(ConsoleIO_Red);
...
static int32_t MenuSelection(void)
{
    ...
    printf("53-Run a Parameter Sweep\n");
    printf("54-Set all Advanced Parameters\n");
...
static void ParameterSweepLong(uint8_t Address, uint16_t* param_nums, uint32_t* param_vals)
{
    ...
    for (i = 0; i < 39; i++){
        if (MeCom_ParValuel(Address,param_nums[i], 1, &Fields, MeGet)){
            param_vals[i] = Fields.Value;
        }
        else{
            printf("Long Parameter %d not available\n", param_nums[i]);
        }
    }
 ...
 static void ParameterSweepFloat(uint8_t Address, uint16_t* param_nums, float* param_vals)
{
    ...
    for (i = 0; i < 68; i++){
        if(MeCom_ParValuef(Address,param_nums[i], 1, &Fields, MeGet)){
            param_vals[i] = Fields.Value;
        }
        else{
            printf("Float Parameter %d %i not available\n", param_nums[i], i);
        }
...
static void ParameterSetLong(uint8_t Address, uint16_t* param_nums, uint32_t* param_vals)
{
    ...
    for (i = 0; i < 39; i++){
        ...
        if(MeCom_ParValuel(Address,param_nums[i], 1, &Fields, MeSet))
 ...
 static void ParameterSetFloat(uint8_t Address, uint16_t* param_nums, float* param_vals)
{
    ...
    for (i = 0; i < 68; i++){
        Fields.Value = param_vals[i];
        if(MeCom_ParValuef(Address,param_nums[i], 1, &Fields, MeSet))
            printf("Wrote Successfully!!\n");
        else
            printf("FAILED!!!! %d %d\n", param_nums[i], i);
    }
